...
SELECT
    ...
    T2._Fld23568RRef AS Склад�?сточника,
...
SELECT
    T1._Fld23831RRef AS Склад�?сточника,
...
WHERE
    T1._Fld23831RRef IN (
        SELECT
            T2.Склад�?сточника AS Склад�?сточника
        FROM
...
FROM
    ...
    ON (T4.Склад�?сточника = T5.Склад�?сточника)
...
FROM
    ...
    ON (T6.Склад�?сточника = T7.Склад�?сточника)
    AND (T6.ДатаСобытия = T7.ДатаСобытия)
WHERE
    NOT T6.�?сточник_RRRef IS NULL
...
FROM
    ...
    AND (
        ...
        AND T1.�?сточник_RTRef = Резервирование._RecorderTRef
        AND T1.�?сточник_RRRef = Резервирование._RecorderRRef
    )
;
...
SELECT
    ...
    MIN(T1.Цена) AS Цена�?сточника,
    MIN(T1.Цена / 100.0 * (100 - 3.0)) AS Цена�?сточникаМинус --это параметр ПроцентДнейАнализа
FROM
...
FROM
    ...
    AND (T1.Цена <= T2.Цена�?сточникаМинус)
    AND (T1.Цена <> 0)
GROUP BY
    T2.НоменклатураСсылка,
    T2.СкладНазначения,
